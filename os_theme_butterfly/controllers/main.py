...
from odoo.addons.web.controllers import home as web
...
class MainController(Controller):
    ...
    def get_installed_languages(self):
        ...
        """
        langs = request.env['res.lang'].with_context(active_test=True, lang=request.env.user.lang).search([])
    ...
    def userSaveThemeSettings(self, data):
        all_users = request.env['res.users'].sudo().search([('active', '=', True)])
        all_users.write(data)
