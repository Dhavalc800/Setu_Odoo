...
import {OsBookmarkDropdown} from "@os_theme_butterfly/components/os_bookmark/os_bookmark_dropdown";
import {useService} from "@web/core/utils/hooks";
import {browser} from "@web/core/browser/browser";

const {Component, useRef, onWillStart} = owl;
...
export class OsBookmarkMenu extends Component {
    ...
    enableEditMode() {
        $(this.rootRef.el).addClass("edit_mode");
    }

    disableEditMode() {
        $(this.rootRef.el).removeClass("edit_mode");
    }
    ...
    updateBookmarkItem(ev) {
        var $el = $(ev.currentTarget);
        var self = this;
        let name = $(this.rootRef.el.querySelector('#bookmark_name')).val().trim();
        let description = $(this.rootRef.el.querySelector('#bookmark_description')).val().trim();
        let icon = $(this.rootRef.el.querySelector('#bookmark_icon')).val().trim();
        let sequence = $(this.rootRef.el.querySelector('#bookmark_sequence')).val().trim();

        if (name === '' || icon === '') {
            $(this.rootRef.el.querySelector('#bookmark_item_errors_msg')).html(
    ...
    addBookmarkItem(ev) {
        ...
        let description = $(this.rootRef.el.querySelector('#bookmark_description')).val().trim();
        let icon = $(this.rootRef.el.querySelector('#bookmark_icon')).val().trim();
        let link = $(this.rootRef.el.querySelector('#bookmark_link_external')).val().trim();
        let sequence = $(this.rootRef.el.querySelector('#bookmark_sequence')).val().trim();
        ...
        if (name === '' || icon === '' || link === '') {
            ...
            return this.rpc("/theme/bookmark/add", {data: data}).then(function (data) {
                if (data[0]) {
                    $(self.rootRef.el.querySelector('#bookmark_item_errors_msg')).html(
                        $('<div class="alert alert-danger py-1 fs-14px mb-2" role="alert"/>')
                            .text(data[0])
                    );
                } else {
