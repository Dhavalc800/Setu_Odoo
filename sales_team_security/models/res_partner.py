...
class SaleOrder(models.Model):
    ...
    def _get_view(self, view_id=None, view_type='form', **options):
        ...
        if not self.env.user.has_group('sales_team_security.group_booking_invoice_readonly'):
            for node in arch.iterfind(".//button[@name='action_view_invoice']"):
                node.set("invisible", "1")
            for node in arch.iterfind(".//field[@name='invoice_count']"):
                node.set("invisible", "1")
        return arch, view
