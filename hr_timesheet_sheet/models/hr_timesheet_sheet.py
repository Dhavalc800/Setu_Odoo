...
class Sheet(models.Model):
    ...
    def _compute_name(self):
        ...
        for sheet in self:
            if sheet.date_start and sheet.date_end and sheet.date_start == sheet.date_end:
            ...
            period_start = sheet.date_start and sheet.date_start.strftime("%V, %Y")
            period_end = sheet.date_end and sheet.date_end.strftime("%V, %Y")

            if  sheet.date_start and sheet.date_end and sheet.date_end <= sheet.date_start + relativedelta(weekday=SU):
    ...
    def _check_start_end_dates(self):
        for sheet in self:
            if sheet.date_start and sheet.date_start and  sheet.date_start > sheet.date_end:
    ...
    def _compute_line_ids(self):
        ...
        for sheet in self:
            ...
            SheetLine_ids = SheetLine.create(vals_list).ids
            sheet.line_ids = [(6, 0, SheetLine_ids)] or []
    ...
    def _get_subscribers(self):
        """ Hook for extensions """
        self.ensure_one()
        partner_id = self.employee_id.parent_id.user_id.partner_id
        subscribers = self.sudo().employee_id.parent_id.user_id.partner_id
    ...
    def action_timesheet_draft(self):
        if self.filtered(lambda sheet: sheet.state not in ("confirm","done")):
    ...
    def action_submit_multiple_timesheet(self):
        self = self.sudo()
        for rec in self:
            if rec.state not in ('new', 'draft'):
                raise ValidationError(
                "You can submit Timesheet in New or Draft state")
            rec.action_timesheet_confirm()

    def action_approve_multiple_timesheet(self):
        self = self.sudo()
        for rec in self:
            if rec.state in ('new', 'done'):
                raise ValidationError(_(
                "You can not Approve Timesheet in %s state") % (rec.state))
            rec.action_timesheet_done()
            
    def action_timesheet_confirm(self):
        self._timesheet_subscribe_users()
        self.reset_add_line()
        if self.timesheet_ids:
            for sheet_id in self.timesheet_ids:
                if sheet_id.name and len(sheet_id.name) == 1 and sheet_id.name =='/':
                    raise ValidationError(
                "You can not submit Timesheet having Description only '/' ")
    ...
    def action_timesheet_done(self):
        ...
        if self._uid != 2 and self.employee_id.user_id.id == self._uid:
            raise ValidationError(
                'You can not approve your own Timesheet request!')
    ...
    def action_timesheet_refuse(self):
        ...
        # if self._uid != 1 and self.employee_id.user_id.id == self._uid:
        #     raise ValidationError('You can not approve your own Timesheet request!')
        # self._check_can_review()
        # self.write({"state": "draft", "reviewer_id": False})
        return {
            'type': 'ir.actions.act_window',
            'res_model': 'timesheet.refuse.reason',
            'view_mode': 'form',
            'name': "Timesheet Refuse Reason",
            'views': [(False, 'form')],
            'target': 'new',
        }
