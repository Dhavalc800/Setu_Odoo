...
class IrAttachment(models.Model):
    ...
    def _get_s3_bucket(self, name=None):
        ...
        * ``AWS_BUCKETNAME``
        ...
        # access_key = os.environ.get("aws3_access_key_ID")
        # secret_key = os.environ.get("AWS_SECRET_ACCESS_KEY")
        bucket_name = name or os.environ.get("AWS_BUCKETNAME")
        # replaces {db} by the database name to handle multi-tenancy

        print ("\n bucket_namebucket_name",bucket_name)

        region_name = self.env['ir.config_parameter'].sudo().get_param('aws3_region_name')
        access_key = self.env['ir.config_parameter'].sudo().get_param('aws3_access_key')
        secret_key = self.env['ir.config_parameter'].sudo().get_param('aws3_secret_key')
        bucket_name = self.env['ir.config_parameter'].sudo().get_param('aws3_bucket_name')
        ...
        # if not host:
        params["endpoint_url"] = "https://object.ishancloud.com/"
        if region_name:
