...
import {OS_Favorite_Apps_Dropdown} from "@os_theme_butterfly/components/os_favorite_app/os_favorite_apps_dropdown";
...
export class OsFavoriteAppMenu extends Component {
        ...
        onPatched(() => {
            ...
            if (this.action === "add_favorite_app_item" || this.action === "edit_favorite_app_item") {
                ...
                this.$select2 = $input.select2({
                    width: '100%',
                    dropdownCssClass: "os-favorite-app-dropdown",
                    allowClear: true,
    ...
    updateFavoriteAppItem(ev) {
        var self = this;
        var $el = $(ev.currentTarget);
        let name = $(this.rootRef.el.querySelector('#favorite_app_name')).val().trim();
        let sequence = $(this.rootRef.el.querySelector('#favorite_app_sequence')).val().trim();
        if (name === '') {
            $(self.rootRef.el.querySelector('#favorite_app_errors_msg')).html(
    ...
    async addFavoriteAppItem() {
        var self = this;
        let name = $(this.rootRef.el.querySelector('#favorite_app_name')).val().trim();
        let menu_id = $(this.rootRef.el.querySelector('#application')).val().trim();
        let sequence = $(this.rootRef.el.querySelector('#favorite_app_sequence')).val().trim();
        if (name === '' || menu_id === '') {
            $(self.rootRef.el.querySelector('#favorite_app_errors_msg')).html(
                $('<div class="alert alert-danger py-1 fs-14px mb-2" role="alert"/>')
