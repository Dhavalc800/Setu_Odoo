...
import {OsQuickCreate_Dropdown} from "@os_theme_butterfly/components/os_quick_create/os_quick_create_dropdown";
...
export class OsQuickCreate extends Component {
        ...
        onPatched(() => {
            ...
            if (this.action === "add_quick_create_item" || this.action === "edit_quick_create_item") {
                ...
                this.$select2 = $input.select2({
                    width: '100%',
                    dropdownCssClass: "os-quick-create-dropdown",
                    allowClear: true,
    ...
    updateQuickCreateItem(ev) {
        var self = this;
        var $el = $(ev.currentTarget);
        let name = $(this.rootRef.el.querySelector('#quick_create_name')).val().trim();
        let model = $(this.rootRef.el.querySelector('#model')).val().trim();
        let sequence = $(this.rootRef.el.querySelector('#quick_create_sequence')).val().trim();
        let icon = $(this.rootRef.el.querySelector('#quick_create_icon')).val().trim();
        if (name === '' || model === '' || icon === '') {
            $(self.rootRef.el.querySelector('#quick_create_errors_msg')).html(
