...
export class OsSettingsPanelUser extends Component {
    ...
    onMounted() {
        ...
        let theme_mode_opt = this.rootRef.el.querySelector('[data-key="mode"].active')
        let theme_mode = $(theme_mode_opt).data("update");
        $(this.sidebar_style_ref.el).toggle($(this.rootRef.el.querySelector('[data-key="navigation_style"].active')).attr('data-update') === 'vertical');
        ...
        this.$select2 = $input.select2({
         ...
         if (this.env.services.user.home_action_id) {
            $input.select2('data', {id: String(this.env.services.user.home_action_id), text: this.themeService.os_home_action_name});
        }

    }

    onChangeDivSameIcons() {
        let condition = $(this.rootRef.el.querySelector('[data-key="navigation_style"].active')).attr('data-update') === 'vertical';
    ...
    async userSaveThemeSettings() {
        var self = this;
        framework.blockUI();
        let header_style_opt = this.rootRef.el.querySelector('[data-key="header"].active');
        let sidebar_style_opt = this.rootRef.el.querySelector('[data-key="aside"].active');
        let chatter_position_opt = this.rootRef.el.querySelector('[data-key="os_chatter_position"].active');
        let dbl_click_edit_opt = this.rootRef.el.querySelector('#user_dbl_click_edit');
        let display_todo_list_opt = this.rootRef.el.querySelector('#user_display_todo_list');
        let display_bookmarks_opt = this.rootRef.el.querySelector('#user_display_bookmarks');
        let display_favorite_apps_opt = this.rootRef.el.querySelector('#user_display_favorite_apps');
        let display_recently_viewed_records_opt = this.rootRef.el.querySelector('#user_display_recently_viewed_records');
        let display_zoom_in_out_opt = this.rootRef.el.querySelector('#user_display_zoom_in_out');
        let display_full_screen_opt = this.rootRef.el.querySelector('#user_display_full_screen');
        let home_action_input_opt = this.rootRef.el.querySelector('#home_action_input');
        let os_show_sidebar_opt = this.rootRef.el.querySelector('[data-key="navigation_style"].active');
        let os_show_quick_create_opt = this.rootRef.el.querySelector('#user_show_quick_create');
        let same_icons_opt = this.rootRef.el.querySelector('#user_same_icons');
        let user_header_tools_bar_fixed_opt = this.rootRef.el.querySelector('#user_header_tools_bar_fixed');
...
export class OsSettingsPanelGeneral extends Component {
    ...
    toggleTabContent(ev) {
        ...
        var $parent = $el.parents(".nav-tabs");
        $parent.find('a[href="' + $el.attr("href") + '"]').tab('show');
    }
    ...
    onMounted() {
        ...
        $(self.sidebar_style_ref.el).toggle($(this.rootRef.el.querySelector('[data-key="navigation_style"].active')).attr('data-update') === 'vertical');
        ...
        $(this.rootRef.el).find('.os_widget_color').each(function () {
            $(this).minicolors({
                control: 'hue',
                inline: false,
    ...
    toggleShowRibbon() {
        let selector_val = $(this.rootRef.el.querySelector('#os_activate_web_ribbon')).is(':checked');
        $(this.ribbon_bloc_ref.el).toggle(selector_val);
    }

    onChangeCustomBaseFont() {
        let condition = $(this.rootRef.el.querySelector('#os_theme_base_font')).val() === "custom";
        if (condition && $(this.rootRef.el.querySelector('#os_theme_base_custom_google_font')).val() === "") {
            $(this.rootRef.el.querySelector('#fonts_base_msg_errors')).html(
    ...
    onChangeCustomaAltFont() {
        let condition = $(this.rootRef.el.querySelector('#os_theme_alt_font')).val() === "custom";
        if (condition && $(this.rootRef.el.querySelector('#os_theme_alt_custom_google_font')).val() === "") {
            $(this.rootRef.el.querySelector('#fonts_alt_msg_errors')).html(
                $('<div class="alert alert-warning py-1 fs-14px mb-2" role="alert"/>')
    ...
    onChangeShape() {
        let condition = $(this.rootRef.el.querySelector('#os_shape')).val() !== "none";
    ...
    onChangeChartPalette() {
        let os_chart_color_palette = $(this.rootRef.el.querySelector('#os_chart_color_palette')).val();
        $(this.chart_bg_palette_ref.el).attr('class', 'chart_bg_' + os_chart_color_palette);

    }

    onChangeShapeStyle() {
        let condition = $(this.rootRef.el.querySelector('#os_shape_style')).val() === "unique_color" && $(this.rootRef.el.querySelector('#os_shape')).val() !== "none";
        $(this.shape_style_unique_color_ref.el).toggle(condition);
    }
    ...
    onChangeAnimationModalEntrance() {
        let condition = $(this.rootRef.el.querySelector('#os_modal_animated_entrance')).val() === 'True'
    ...
    onChangeLoginStyle() {
        ...
        let condition2 = ["default", "style_1", "style_2", "style_3", "style_4", "style_5", "style_6"].indexOf($(this.rootRef.el.querySelector('#os_login_style')).val()) > -1
        ...
        let condition3 = ["style_6"].indexOf($(this.rootRef.el.querySelector('#os_login_style')).val()) > -1
        if (condition3) {
        ...
        let condition = ["style_4", "style_5"].indexOf($(this.rootRef.el.querySelector('#os_login_style')).val()) > -1
    ...
    onChangeIconStyle() {
        let condition = $(this.rootRef.el.querySelector('#os_apps_icon_style')).val() === "font";
        $(this.shape_div_ref.el).toggle(condition);
        let condition_2 = $(this.rootRef.el.querySelector('#os_apps_icon_style')).val() === "image";
        $(this.config_style_image_ref.el).toggle(condition_2);
    }

    toggleLoginType() {
        let selector_val = $(this.rootRef.el.querySelector('#os_login_background_type')).val();
    ...
    async changeColor() {
        const colorInput = this.input_primary_color_ref.el.value;
        const isValidHex = /^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$/i.test(colorInput);
        
        if (!isValidHex) {
            $(this.input_primary_color_ref.el).addClass('is-invalid');
            if (!$(this.input_primary_color_ref.el).next('.invalid-feedback').length) {
                $(this.input_primary_color_ref.el).after('<div class="invalid-feedback">Please enter a valid hex color code (e.g. #875A7B)</div>');
            }
            return false;
        }
        
        $(this.input_primary_color_ref.el).removeClass('is-invalid');
        $(this.input_primary_color_ref.el).next('.invalid-feedback').remove();
        
        $(this.opt_custom_color_ref.el).attr("data-update", colorInput);
    }
    ...
    async companySaveThemeSettings() {
        ...
        let sidebar_style_opt = this.rootRef.el.querySelector('[data-key="aside"].active');
        let theme_mode_opt = this.rootRef.el.querySelector('[data-key="mode"].active');
        let theme_color_primary_opt = this.rootRef.el.querySelector('[data-key="skin"].active');
        let theme_custom_color_opt = this.rootRef.el.querySelector('[data-custom="true"]');
        let chatter_position_opt = this.rootRef.el.querySelector('[data-key="os_chatter_position"].active');
        let loader_style_opt = this.rootRef.el.querySelector('[data-key="os_loader_style"].active');
        let show_user_settings_opt = this.rootRef.el.querySelector('#os_show_user_settings');
        let dbl_click_edit_opt = this.rootRef.el.querySelector('#os_dbl_click_edit');
        let display_todo_list_opt = this.rootRef.el.querySelector('#os_display_todo_list');
        let display_bookmarks_opt = this.rootRef.el.querySelector('#os_display_bookmarks');
        let display_favorite_apps_opt = this.rootRef.el.querySelector('#os_display_favorite_apps');
        let display_recently_viewed_records_opt = this.rootRef.el.querySelector('#os_display_recently_viewed_records');
        let display_zoom_in_out_opt = this.rootRef.el.querySelector('#os_display_zoom_in_out');
        let display_full_screen_opt = this.rootRef.el.querySelector('#os_display_full_screen');
        let web_window_title_opt = this.rootRef.el.querySelector('[data-key="os_web_window_title"] input');
        let activate_web_ribbon_opt = this.rootRef.el.querySelector('#os_activate_web_ribbon');
        let separator_style_opt = this.rootRef.el.querySelector('#os_separator_style');
        let separator_color_opt = this.rootRef.el.querySelector('#os_separator_color');
        let breadcrumb_style_opt = this.rootRef.el.querySelector('[data-key="os_breadcrumb_style"].active');
        //login page
        let os_login_style_opt = this.rootRef.el.querySelector('#os_login_style');
        let os_login_title_opt = this.rootRef.el.querySelector('#os_login_title');
        let os_login_subtitle_opt = this.rootRef.el.querySelector('#os_login_subtitle');
        let os_login_background_type_opt = this.rootRef.el.querySelector('#os_login_background_type');
        let login_background_color_opt = this.rootRef.el.querySelector('#os_login_background_color');
        // Apps view
        let apps_view_background_type_opt = this.rootRef.el.querySelector('#os_apps_view_background_type');
        let apps_view_background_color_opt = this.rootRef.el.querySelector('#os_apps_view_background_color');
        let apps_view_text_color_opt = this.rootRef.el.querySelector('#os_apps_view_text_color');
        let os_header_tools_bar_fixed_opt = this.rootRef.el.querySelector('#os_header_tools_bar_fixed');

        // Fonts
        let theme_base_font_opt = this.rootRef.el.querySelector('#os_theme_base_font');
        let theme_alt_font_opt = this.rootRef.el.querySelector('#os_theme_alt_font');
        let tabs_style_opt = this.rootRef.el.querySelector('#os_tabs_style');
        let tabs_alignment_opt = this.rootRef.el.querySelector('#os_tabs_alignment');
        let buttons_style_opt = this.rootRef.el.querySelector('#os_buttons_style');
        let buttons_angles_opt = this.rootRef.el.querySelector('#os_buttons_angles');
        let buttons_size_opt = this.rootRef.el.querySelector('#os_buttons_size');
        let radios_style_opt = this.rootRef.el.querySelector('#os_radios_style');
        let checkbox_style_opt = this.rootRef.el.querySelector('#os_checkbox_style');
        let list_view_style_opt = this.rootRef.el.querySelector('#os_list_view_style');
        let os_list_view_header_fg_color_opt = this.rootRef.el.querySelector('#os_list_view_header_fg_color');
        let os_list_view_header_bg_color_opt = this.rootRef.el.querySelector('#os_list_view_header_bg_color');
        let os_shape_opt = this.rootRef.el.querySelector('#os_shape');
        let os_shape_style_opt = this.rootRef.el.querySelector('#os_shape_style');
        let os_shape_none_color_icon_opt = this.rootRef.el.querySelector('#os_shape_none_color_icon');
        let os_shape_shadow_opt = this.rootRef.el.querySelector('#os_shape_shadow');
        let os_shape_style_unique_color_opt = this.rootRef.el.querySelector('#os_shape_style_unique_color');
        let os_shape_style_unique_color_icon_opt = this.rootRef.el.querySelector('#os_shape_style_unique_color_icon');
        let same_icons_opt = this.rootRef.el.querySelector('#same_icons');
        let os_apps_icon_style_opt = this.rootRef.el.querySelector('#os_apps_icon_style');
        let os_show_sidebar_opt = this.rootRef.el.querySelector('[data-key="navigation_style"].active');
        let os_show_quick_create_opt = this.rootRef.el.querySelector('#os_show_quick_create');
        let os_chart_color_palette_opt = this.rootRef.el.querySelector('#os_chart_color_palette');
        let os_apps_icon_style_image_pack_opt = this.rootRef.el.querySelector('#os_apps_icon_style_image_pack');
        let os_list_view_style_spaced_opt = this.rootRef.el.querySelector('#os_list_view_style_spaced');
        let os_list_view_style_separate_opt = this.rootRef.el.querySelector('#os_list_view_style_separate');
        let os_list_view_style_hover_opt = this.rootRef.el.querySelector('#os_list_view_style_hover');
        let os_modal_animated_entrance_opt = this.rootRef.el.querySelector('#os_modal_animated_entrance');
        let os_modal_animated_entrance_value_opt = this.rootRef.el.querySelector('#os_modal_animated_entrance_value');
        let os_list_view_sticky_header_footer_opt = this.rootRef.el.querySelector('#os_list_view_sticky_header_footer');
        ...
        if ($(theme_base_font_opt).val() === "custom") {
            data_company['os_theme_base_custom_google_font'] = $(this.rootRef.el.querySelector('#os_theme_base_custom_google_font')).val();
        }
        if ($(theme_alt_font_opt).val() === "custom") {
            data_company['os_theme_alt_custom_google_font'] = $(this.rootRef.el.querySelector('#os_theme_alt_custom_google_font')).val();
        }
        if ($(activate_web_ribbon_opt).is(':checked')) {
            data_company['os_web_ribbon_text'] = $(this.rootRef.el.querySelector('#os_web_ribbon_text')).val();
            data_company['os_web_ribbon_bg'] = $(this.rootRef.el.querySelector('#os_web_ribbon_bg')).val();
            data_company['os_web_ribbon_fg'] = $(this.rootRef.el.querySelector('#os_web_ribbon_fg')).val();
        }
