...
class CommissionReportWizard(models.TransientModel):
    ...
    def sales_team_constrains(self):
        ...
        for rec in self:
            if self.sales_team_ids:
                ...
                if (
                    not self.sales_team_ids.member_ids.commission_id
                    and not self.sales_team_ids.commission_id
                ):
                    raise ValidationError(
    ...
    def print_xls_report(self):
        ...
        for user, user_sale_orders in user_sale_orders_dict.items():
            ...
            filtered_order_lines = user_sale_orders.filtered(
                ...
                and l.date_order.date() >= commission_id.date_from
            ).mapped("order_line")
            filtered_order_refund = user_sale_orders.filtered(
                lambda l: self.date_from <= l.date_order.date() <= self.date_to
                and l.date_order.date() >= commission_id.date_from
            )
            person_refund_total = 0
            for order in filtered_order_refund:
                if order.invoice_ids.filtered(lambda i: i.move_type == "out_refund"):
                    person_refund_total += sum(
            ...
            if commission_id.type == "product":
                ...
                for rule in commission_id.product_comm_ids.filtered(
                    ...
                    if total_price - person_refund_total > 0:
                        total_list.append(total_price - person_refund_total)
            ...
            if commission_id.type == "revenue" and (
                ...
                for rule in commission_id.revenue_grd_comm_ids:
                    if filtered_order_lines_commission_total - person_refund_total > 0:
                        if (
                            rule.amount_from
                            <= (
            ...
            if commission_id.type == "revenue" and (
                ...
                straight_commission = (
                    (filtered_order_lines_commission_total - person_refund_total)
        ...
        for team_user in team_obj:
            team_sale_orders_dict.update(
                {team_user: team_sale_orders.filtered(lambda l: l.user_id == team_user)}
            )
        for team_user, team_sale_orders in team_sale_orders_dict.items():
            commissions_id = (
                team_user.commission_id
                if team_user.commission_id
            ...
            if commissions_id:
                ...
                filtered_order_refund = team_sale_orders.filtered(
                    lambda l: self.date_from <= l.date_order.date() <= self.date_to
                    and l.date_order.date() >= commissions_id.date_from
                )
                ...
                for order in filtered_order_refund:
                    if order.invoice_ids.filtered(
                        lambda l: l.move_type == "out_refund"
                    ):
                        refund_total += sum(
                            order.invoice_ids.filtered(
                                lambda i: i.move_type == "out_refund"
                ...
                filtered_order_lines_commission_total = (
                    sum(filtered_order_lines.mapped("price_subtotal")) - refund_total
                )
                if commissions_id.type == "product":
                    ...
                    for rules in commissions_id.product_comm_ids.filtered(
                        ...
                        if total_price - refund_total > 0:
                            commission_total.append(total_price - refund_total)
                ...
                if commissions_id.type == "revenue" and (
                    ...
                    for rules in commissions_id.revenue_grd_comm_ids:
                        if (
                            ...
                            graduated_commission = (
                                (filtered_order_lines_commission_total - refund_total)
                                * rules.graduated_commission_rate
                            ) / 100
    ...
    def get_xlsx_report(self, data, response):
        ...
        head = workbook.add_format(
            {"align": "center", "bold": True, "font_size": "15px", "valign": "vcenter"}
        )
        ...
        heading = workbook.add_format(
            {"align": "left", "bold": True, "font_size": "12px", "valign": "vcenter"}
        )
        ...
        if user:
            ...
            sheet.write("D4", "Total Untaxed Revenue", heading)
            ...
            sheet.write(row + 1, col + 4, round(sum(commission_list), 2), format2)
        elif team:
            ...
            sheet.write("E4", "Total Untaxed Revenue", heading)
            ...
            sheet.write(row + 1, col + 4, round(sum(commission_total), 2), format2)
