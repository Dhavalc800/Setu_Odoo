...
class CRMLead(models.Model):
    ...
    def get_the_sales_activity(self):
        ...
        self._cr.execute('''select (mail_activity_type.name->>%s) as name,COUNT(*) from mail_activity
        inner join mail_activity_type on mail_activity.activity_type_id = mail_activity_type.id
        where mail_activity.res_model = 'crm.lead' GROUP BY mail_activity_type.name''', [lang])
    ...
    def revenue_count_pie(self):
        ...
        total_expected_revenue = [item['revenue'] for item in
                                  total_expected_revenue_data]
    ...
    def get_monthly_goal(self):
        ...
        total = 0
        for rec in leads.filtered(
                lambda t: t.date_deadline.month == fields.date.today().month):
    ...
    def get_total_lost_crm(self, option):
        ...
        if option == '1':
            ...
            for rec in data:
                datetime_object = datetime.strptime(str(int(rec['date_part'])),
                                                    "%m")
    ...
    def get_ratio_based_country(self):
        ...
        country_wise_ratio = sorted(country_wise_ratio, key=lambda x: x[3],
                                    reverse=True)
    ...
    def get_lost_lead_by_reason_pie(self):
        ...
        self._cr.execute('''select lost_reason_id, count(*), (SELECT name FROM crm_lost_reason
        WHERE id = lost_reason_id) from crm_lead where probability=0 and active=false and
        type='lead' group by lost_reason_id''')
    ...
    def crm_year(self):
        ...
        if revenue_value > 0:
            ...
            print(ratio_value, 'ratio_value1')
            ratio_value = str(ratio_value)[1:-1]
            print(ratio_value, 'ratio_value2')
